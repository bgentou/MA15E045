...
Ce jeu de données est construit à partir du package {{< fa brands r-project >}} [`nycflights13`](https://github.com/tidyverse/nycflights13){target="_blank"}. 
...
![Schema `nycflights` Generated using [DbSchema](https://dbschema.com)](/images/avt-2025/schemas/schema-nycflights.png)
...
{{< fa hand-point-right >}} `distance` est la distance orthodromique  (*earth distance*) entre `origin`  et `dest` en miles (pas en miles nautiques). On peut le vérifier avec les fonctions de l'extension `earthdistance`. 

{{< fa hand-point-right >}} le vol est effectué si `dep_time` n'est pas `NULL`

{{< fa hand-point-right >}} `dep_time`, `sched_dep_time`, `arr_time`, `sched_arr_time` sont construits de la façon 
